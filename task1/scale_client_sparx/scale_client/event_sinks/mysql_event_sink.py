...
class MySQLEventSink(EventSink):
	...
	class EventRecord(peewee.Model):
		...
		sensor = peewee.CharField(max_length = 64)
	...
	def _create_table(self, record_kls=None):
		...
		try:
			...
			log.info("created table: "+ str(record_kls._meta.table_name))
	...
	def send_raw(self, encoded_event):
		try:
			if isinstance(encoded_event, self.EventRecord):
				encoded_event.save()
				log.info("MySQL record saved to " + str(self.EventRecord._meta.table_name))
				return True
